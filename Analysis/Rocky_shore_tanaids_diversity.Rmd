...
r2 <- round(summary(lm(qD~Mean, data=t1))$adj.r.squared, 3)
label = paste0("P==", pv, "*','~italic(R)^2 ==", r2)
...
p8 <- ggplot(data=t1, aes(x=Mean, y=qD, ymin=qD.LCL, ymax=qD.UCL, shape=Season, colour=Site))+
  ...
  geom_errorbar()+
  stat_smooth(formula=y~x, method = "glm", aes(group=1), alpha=0.2, colour="gray", linetype=2)+
  annotate(geom = "text", x = 25, y = 22, hjust = 0, vjust = 1, label = label, parse = TRUE) +
  scale_color_viridis_d()+
  scale_shape_manual(values=c(1,19))+
...
r2 <- round(summary(lm(qD~Mean, data=t2))$adj.r.squared, 3)
label = paste0("P==", pv, "*','~italic(R)^2 ==", r2)
...
p9 <- ggplot(data=t2, aes(x=Mean, y=qD, ymin=qD.LCL, ymax=qD.UCL, shape=Season, colour=Site))+
  ...
  geom_errorbar()+
  stat_smooth(formula=y~x, method = "glm", aes(group=1), alpha=0.2, colour="gray", linetype=2)+
  annotate(geom = "text", x = 25, y = 6, hjust = 0, vjust = 1, label = label, parse = TRUE) +
  scale_color_viridis_d()+
  scale_shape_manual(values=c(1,19))+
...
r2 <- round(summary(lm(H2~Mean, data=h2))$adj.r.squared, 3)
label = paste0("P==", pv, "*','~italic(R)^2 ==", r2)
...
p10<- ggplot(data=h2, aes(x=Mean, y=H2, ymin=X1, ymax=X2, shape=Season, colour=Site))+
  ...
  annotate(geom = "text", x = 25, y = 0.95, hjust = 0, vjust = 1, label = label, parse = TRUE) +
  scale_color_viridis_d()+
  scale_shape_manual(values=c(1,19))+
