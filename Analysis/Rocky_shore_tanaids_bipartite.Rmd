...
```{r}
ta2 <- aggregate(ta[, -1:-4], by = list(ta$Site, ta$Month, ta$Microhabitat), FUN=mean)
names(ta2)[1:3] <- c("Site", "Month", "Microhabitat")
b2 <- ta2[, -1:-3]
rownames(b2) <- with(ta2, paste(Site, Month, Microhabitat, sep="_"))
```

# Converts observations into a network matrix

```{r}
out <- melt(ta2, id.vars = c("Site", "Month", "Microhabitat"), variable.name = "Species", value.name = "Abundance")

# Separate by Months
sp <- subset(out, Month=="4")
su <- subset(out, Month=="8")
...
H2fun_b <- function(d, i){
  avg <- aggregate(d[i, -1:-4], by = list(d$Site, d$Month, d$Microhabitat), FUN=mean)
  names(avg)[1:3] <- c("Site", "Month", "Microhabitat")
  d2 <- melt(avg, id.vars = c("Site", "Month", "Microhabitat"), variable.name = "Species", value.name = "Abundance")
  ...
  out <- c(H2fun(web$A, H2_integer=FALSE)[1], 
         H2fun(web$B, H2_integer=FALSE)[1], 
         H2fun(web$C, H2_integer=FALSE)[1])
  return(out)
}

set.seed(200)
sp_h2 <- boot(subset(ta, Month=="4"), H2fun_b, R=1000)
su_h2 <- boot(subset(ta, Month=="8"), H2fun_b, R=1000)
...
out <- cbind(Site = c("A", "B", "C", "A", "B", "C"), Month = c("4", "4", "4", "8", "8", "8"),
  data.frame(
...
```{r}
out$Site <- factor(out$Site, levels=c("A", "B", "C"))
ggplot(data=out, aes(x=Mean, y=H2, ymin=X1, ymax=X2, shape=Month, colour=Site))+
...
p1 <- b_fun(create_layout(sp_web$A, "bipartite")) + labs(tag = "a", caption = "(A-4)") 
# For some reason, I need to add a "blank" value to make create_layout working.
p2 <- b_fun(create_layout(cbind(su_web$A, 0), "bipartite")[-11,]) + labs(tag = "b", caption = "(A-8)")

p3 <- b_fun(create_layout(sp_web$B, "bipartite")) + labs(tag = "c", caption = "(B-4)") 
p4 <- b_fun(create_layout(su_web$B, "bipartite")) + labs(tag = "d", caption = "(B-8)") 

p5 <- b_fun(create_layout(sp_web$C, "bipartite")) + labs(tag = "e", caption = "(C-4)") 
p6 <- b_fun(create_layout(su_web$C, "bipartite")) + labs(tag = "f", caption = "(C-8)") 
```
...
p1 <- ggplot(data.frame(sp_d[[1]]),aes(x=Species, y=Link))+
  geom_bar(stat="identity")+
  labs(x="", y="No. of link", tag = "a", caption = "(A-4)")+
  theme_bw() %+replace% rotate %+replace% large
...
p3 <- ggplot(data.frame(sp_d[[2]]),aes(x=Species, y=Link))+
  geom_bar(stat="identity")+
  labs(x="", y="No. of link", tag = "c", caption = "(B-4)")+
  theme_bw() %+replace% rotate %+replace% large
...
p5 <- ggplot(data.frame(sp_d[[3]]),aes(x=Species, y=Link))+
  geom_bar(stat="identity")+
  labs(x="", y="No. of link", tag = "e", caption = "(C-4)")+
  theme_bw() %+replace% rotate %+replace% large
...
p1 <- ggplot(data.frame(sp_d[[1]]),aes(x=Species, y=Degree))+
  geom_bar(stat="identity")+
  labs(x="", y="Degree", tag = "a", caption = "(A-4)")+
  theme_bw() %+replace% rotate %+replace% large
...
p3 <- ggplot(data.frame(sp_d[[2]]),aes(x=Species, y=Degree))+
  geom_bar(stat="identity")+
  labs(x="", y="Degree", tag = "c", caption = "(B-4)")+
  theme_bw() %+replace% rotate %+replace% large
...
p5 <- ggplot(data.frame(sp_d[[3]]),aes(x=Species, y=Degree))+
  geom_bar(stat="identity")+
  labs(x="", y="Degree", tag = "e", caption = "(C-4)")+
  theme_bw() %+replace% rotate %+replace% large
...
# Average across collection site and Month
